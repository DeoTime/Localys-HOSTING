...
const threads = [
  { id: 1, community: 'r/RichmondHillEats', title: 'Best hidden-gem bakery near Yonge & 16th?', author: 'localfoodie', votes: 128, comments: 34 },
  { id: 2, community: 'r/LocalServices', title: 'Reliable HVAC for a furnace tune-up before winter?', author: 'homeowner_22', votes: 86, comments: 19 },
  { id: 3, community: 'r/SupportLocal', title: 'Drop your favourite independent coffee shop', author: 'beanlover', votes: 254, comments: 91 },
  { id: 4, community: 'r/Markham', title: 'New taco spot is unreal — birria is a must', author: 'tacotuesday', votes: 173, comments: 47 },
];
            ...
            {threads.map((t) => (
              <div key={t.id} className="flex gap-3 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-900">
                <div className="flex flex-col items-center gap-1 text-black dark:text-white">
                  ...
                  <span className="text-sm font-semibold text-black dark:text-white">{t.votes}</span>
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-xs font-medium text-[#f97316]">{t.community}</p>
                  <p className="truncate font-semibold">{t.title}</p>
                  <p className="mt-1 flex items-center gap-3 text-xs text-black dark:text-white">
